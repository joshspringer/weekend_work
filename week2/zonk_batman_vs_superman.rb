...
class Superhero
  ...
  attr_writer :name, :hitpoints, :alive
  ...
  def hit(target)
    if @has_special_tool
      if (target.hitpoints - (@attack*3)) < 1
        target.hitpoints = 0
        target.alive = false
      else target.hitpoints -= (@attack*3)
      end
    elsif target.hitpoints - @attack < 1
        target.hitpoints = 0
        target.alive = false
      else target.hitpoints -= @attack
    end
  end
